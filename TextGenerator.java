...
public class TextGenerator {
    ...
    public static void main(String[] args) throws IllegalArgumentException {
        ...
        String commandLineInput = readStdInAsString();
        StringBuilder commandLineOutput = new StringBuilder();

        commandLineOutput.append(commandLineInput.substring(0, k));
        MarkovModel markovModel = new MarkovModel(commandLineInput, k);

        while (commandLineOutput.length() < T) {
            String output = commandLineOutput.substring(commandLineOutput.length() - k, commandLineOutput.length());
            char random = markovModel.random(output);
            commandLineOutput.append(random);
        }
